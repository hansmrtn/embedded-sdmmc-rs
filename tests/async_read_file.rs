...
#[test]
fn async_read_file_prime_blocks() {
    pollster::block_on(async {
        let time_source = utils::make_time_source();
        let disk = utils::make_async_block_device(utils::DISK_SOURCE).unwrap();
        let mut volume_mgr = embedded_sdmmc::AsyncVolumeManager::new(disk, time_source);
...
#[test]
fn async_read_file_with_odd_seek() {
    pollster::block_on(async {
        let time_source = utils::make_time_source();
        let disk = utils::make_async_block_device(utils::DISK_SOURCE).unwrap();
        let mut volume_mgr = embedded_sdmmc::AsyncVolumeManager::new(disk, time_source);
        ...
        let f = volume_mgr
            ...
            .await
            .unwrap();
        volume_mgr.file_seek_from_start(f, 0x2c).unwrap();
        while volume_mgr.file_offset(f).unwrap() < 1000000 {
            let mut buffer = [0u8; 2048];
            volume_mgr.read(f, &mut buffer).await.unwrap();
            volume_mgr.file_seek_from_current(f, -1024).unwrap();
        }
    });
}
